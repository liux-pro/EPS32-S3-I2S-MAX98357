...
void calc_file_size(const char *file_path){
    struct stat st;
...
static uint32_t get_file_offset(FILE *file)
{
	return ftell(file);
}
...
uint8_t mp3_id3v2_decode(uint8_t* buf, uint32_t size, MP3CTRL *pctrl) {
	...
	uint32_t tagsize = ((uint32_t)taghead->size[0] << 21)
				  | ((uint32_t)taghead->size[1] << 14)
				  | ((uint16_t)taghead->size[2] << 7)
				  | taghead->size[3];
	...
	while (buf < buff + size) {
		...
		//得到帧大小
		uint32_t frame_size = ((uint32_t)framehead->size[0] << 24)
						 | ((uint32_t)framehead->size[1] << 16)
						 | ((uint32_t)framehead->size[2] << 8)
						 | framehead->size[3];
 ...
 *     其他,失败
 */
uint8_t mp3_get_info(FILE *fmp3, uint8_t *buf, uint32_t size, MP3CTRL *pctrl) {
	...
	printf("%d bp/s audio MPEG layer %d, %d Hz sample rate,  %d Chanel , outputSamps:%d.\n",
	...
	if (strncmp("VBRI", fvbri->id, 4) == 0) {
		...
		//得到总帧数
		totframes = ((uint32_t)fvbri->frames[0] << 24)
					| ((uint32_t)fvbri->frames[1] << 16)
					| ((uint16_t)fvbri->frames[2] << 8)
					| fvbri->frames[3];
		...
		if (strncmp("Xing", (char*)fxing->id, 4) == 0 || strncmp("Info", (char*)fxing->id, 4) == 0) { //是Xing帧
			if (fxing->flags[3] & 0X01) {
				//存在总frame字段
				//得到总帧数
				totframes = ((uint32_t)fxing->frames[0] << 24)
							| ((uint32_t)fxing->frames[1] << 16)
							| ((uint16_t)fxing->frames[2] << 8)
							| fxing->frames[3];
...
void mp3_get_curtime(FILE *fmp3, MP3CTRL *mp3x) {
	uint32_t fpos = 0;
	uint32_t foffset = get_file_offset(fmp3);
